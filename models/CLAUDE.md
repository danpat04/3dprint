...
이 디렉토리(`models/`)는 **사용자가 스펙(요구사항)을 정의하고, Claude가 처음부터 끝까지 모델링/검증을 자율 수행**하는 워크플로 영역입니다. 과거 협업 방식으로 만들던 모델(구 `legacy/`)은 모두 이 `models/` 로 이관 완료되어 `legacy/` 디렉토리는 제거되었습니다. 커피 관련은 `models/coffee/`, 장난감은 `models/toy/` 처럼 카테고리 하위에 모으기도 합니다.
