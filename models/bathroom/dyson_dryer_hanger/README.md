...
**헤드는 90° 돌려** 축이 좌우로 가게 건다. 좌우 겹침 한쪽 12.0mm.

### 걸이

| 항목 | 값 |
|---|---|
| 리브 간격 | **44** (핸들 38 + 여유 3.0) — 실사용 후 41 에서 넓힘 |
| 리브 두께 | 6 |
...
| `connector_a` | 66 × 12.2 × 6 | 5.6 g | 1 |
| `beam` | 66 × 10 × 18 | 13.5 g | 1 |
| **합계** | | **약 78 g** | |
...
**2026-09-26 출력·설치·사용 완료.** 구조·조립·거치 모두 의도대로 동작.
...
41 은 **손잡이를 넣고 빼기가 빡빡했다.** 원통 핸들이라 여유가 적어도 들어갈 거라
봤는데, 매일 한 손으로 하는 동작에서는 1.5mm 로 부족했다.
44 로 넓혀도 헤드 겹침이 12mm 남아 걸림에는 영향이 없다.

> 교훈: **자주 반복하는 동작의 끼움은 계산상 최소치보다 넉넉해야 한다.**
> 한 번 맞추면 끝인 조립부와는 기준이 다르다.

### 십자 맞춤 공차 0.15 → 0.05

A 가 B 보다 눈에 띄게 헐거웠다. 원인은 둘이었다.

| | 물림 깊이 | 총 유격 | 흔들림 각 |
|---|---|---|---|
| A (0.15) | 3mm | 0.30 | 5.7° |
| B (0.10) | 10mm | 0.20 | 1.1° |

공차가 1.5배인데 **물림 깊이가 1/3** 이라 체감은 5배가 됐다.
같은 유격이라도 얕게 물면 흔들림 각이 커진다.
A 를 0.05 로 조여 1.9° 로 내렸다 — B 와 같은 수준.

**조이는 것은 전부 커넥터 쪽에서 했다.** 리브에 파인 홈(12.30)은 그대로 두고
커넥터 몸통을 12.20 으로 두껍게, 커넥터 홈을 6.10 으로 좁게 만들었다.
덕분에 **이미 출력한 리브를 그대로 쓸 수 있다** (`RIB_NOTCH_FIT` 고정).

### 재출력 범위 — 리브는 다시 뽑을 필요가 없다

리브 형상은 `GAP` 과 무관하다. 프로파일은 위치만 옮겨지고, 맞물림 홈은 리브 두께를
관통해 y·z 위치만 쓴다. **간격을 정하는 것은 커넥터와 보다.**
공차도 커넥터 쪽에서만 조였으므로, GAP 41→44 와 공차 0.15→0.05 를 모두 반영해도
**다시 뽑을 것은 `connector_a` 와 `beam` 뿐이다.**
