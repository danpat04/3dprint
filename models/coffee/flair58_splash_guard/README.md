...
**2026-09-25 출력·조립 완료.** 형상·조립·기능은 의도대로 동작.

### ⚠️ 끼움 공차 0.3 은 너무 헐거웠다

| 항목 | 값 |
|---|---|
| 설계값 | `FIT = 0.3` (한쪽당) |
| 실측 결과 | **헐거움** — 텅이 y 방향으로 총 0.6 만큼 논다 |
| 다음 권장 | **0.15~0.2** (0.2 부터 시도) |

설계 당시 "200mm 나 되는 긴 슬라이드 구간이라 여유가 있어야 끝까지 들어간다"고
판단해 넉넉히 잡았는데, **과했다**. 도브테일은 단면이 z 방향으로 일정해서 슬라이드
중에 조여들지 않으므로, 생각보다 타이트하게 잡아도 들어간다.

**값은 0.3 으로 둔다** — 이미 출력한 실물이 0.3 이라 바꾸면 코드와 실물이 어긋난다.
다시 뽑을 일이 생기면 `params.py` 의 `FIT` 한 줄만 고치면 된다.
...
- iter_005: 뒤 플랜지 20 + 쐐기(4.0→1.2) — **최종 · 출력 완료**
  - 출력 후 확인: 끼움 공차 0.3 이 헐거움 (위 "출력 결과" 참고). 재출력은 보류
