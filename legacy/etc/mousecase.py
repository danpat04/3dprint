...
with BuildPart() as part:
    ...
    # 립 바깥 끝에서 안쪽으로 말아내는 처마 (슬라이딩 뚜껑을 +X 방향으로 잡아줌)
    # 위쪽 처마
    with Locations((LID_TIP_X, 0, OUTER_H)):
        Box(
            LID_HOOK_T, OUTER_D, LID_HOOK_DEPTH,
            align=(Align.MAX, Align.CENTER, Align.MAX),
        )
    # 앞쪽 처마
    with Locations((LID_TIP_X, -OUTER_D / 2, 0)):
        Box(
            LID_HOOK_T, LID_HOOK_DEPTH, OUTER_H,
...
with BuildPart() as lid:
    ...
    # 바깥쪽 층 (처마 사이) — 안쪽 층의 +X 끝에 이어 붙임
    with Locations((inner_x_start + LID_INNER_W, 0, 0)):
        Box(
            LID_OUTER_W, LID_OUTER_D, LID_OUTER_H,
            align=(Align.MIN, Align.CENTER, Align.MIN),
        )

    # 바닥 작은 원기둥과 같은 (Y, Z) 위치에 뚜껑 쪽으로도 구멍 (+X 면에서 -X로 2.05mm)
    lid_outer_x_end = inner_x_start + LID_INNER_W + LID_OUTER_W
    with Locations(Plane.YZ.offset(lid_outer_x_end)):
        with Locations(
            (-SMALL_HOLE_Y, HOLE_Z),
            (SMALL_HOLE_Y, HOLE_Z),
        ):
